...
def backprop(x, y, w, learning_rate, n_iter):
    ...
    for i in range(n_iter):
        ...
        #Modify each weight of the output layer by -sig'(output) * loss * hidden_output
        oldw=w.copy()
        ...
        #Calculate hidden_loss as sig'(output) * output_weight for each hidden neuron
        #Modify each weight in the hidden layer by -sig'(hidden_output) * hidden_loss * input features
        # PROBLEM: doesn't backpropagate to hidden layer
        w[0,0] -= sum(sigmoid_derivative(out)*(ypred-y)*oldw[2,0]*sigmoid_derivative(neuron1)*X[:,0])*learning_rate
        w[0,1] -= sum(sigmoid_derivative(out)*(ypred-y)*oldw[2,0]*sigmoid_derivative(neuron1)*X[:,1])*learning_rate
        w[0,2] -= sum(sigmoid_derivative(out)*(ypred-y)*oldw[2,0]*sigmoid_derivative(neuron1))*learning_rate
        w[1,0] -= sum(sigmoid_derivative(out)*(ypred-y)*oldw[2,1]*sigmoid_derivative(neuron2)*X[:,0])*learning_rate
        w[1,1] -= sum(sigmoid_derivative(out)*(ypred-y)*oldw[2,1]*sigmoid_derivative(neuron2)*X[:,1])*learning_rate
        w[1,2] -= sum(sigmoid_derivative(out)*(ypred-y)*oldw[2,1]*sigmoid_derivative(neuron2))*learning_rate
