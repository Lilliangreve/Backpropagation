...
def backpropagation(x, y, w, learning_rate, epochs):
    ...
    for i in range(epochs):
        ...
        #6
        w[0,0] -= sum(sigmoid_derivative(n1) * hidden_loss1 * x[:,0]) * learning_rate
        w[0,1] -= sum(sigmoid_derivative(n1) * hidden_loss1 * x[:,1]) * learning_rate
        w[0,2] -= sum(sigmoid_derivative(n1) * loss * 1.0) * learning_rate
        w[1,0] -= sum(sigmoid_derivative(n1) * hidden_loss1 * x[:,0]) * learning_rate
        w[1,1] -= sum(sigmoid_derivative(n1) * hidden_loss1 * x[:,1]) * learning_rate
